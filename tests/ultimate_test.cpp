...
static std::string extractSessionCookieValue(const std::string &setCookie) {
  std::string key = "SESSION_ID=";
  size_t start = setCookie.find(key);
  if (start == std::string::npos)
    return "";
  start += key.size();
  size_t end = setCookie.find(';', start);
  if (end == std::string::npos)
    return setCookie.substr(start);
  return setCookie.substr(start, end - start);
}
...
int main() {
  ...
  HTTPRequest reqCookie;
  std::string rawCookieReq =
      "GET /index.html HTTP/1.0\r\nHost: localhost\r\nCookie: "
      "SESSION_ID=test123; theme=light\r\n\r\n";
  parser.parseRequest(rawCookieReq, reqCookie);
  assert(reqCookie.getCookie("SESSION_ID") == "test123");
  assert(reqCookie.getCookie("theme") == "light");
  std::cout << "    [OK] Cookie header parsed successfully\n";
  ...
  assert(respGet.headers.find("Set-Cookie") != respGet.headers.end());
  std::string setCookieHeader = respGet.headers["Set-Cookie"];
  std::string createdSessionId = extractSessionCookieValue(setCookieHeader);
  assert(!createdSessionId.empty());
  std::cout << "    [OK] GET handled successfully\n";

  HTTPRequest reqWithSession;
  std::string rawWithSession =
      "GET /index.html HTTP/1.0\r\nHost: localhost\r\nCookie: SESSION_ID=" +
      createdSessionId + "\r\n\r\n";
  parser.parseRequest(rawWithSession, reqWithSession);
  Response respWithSession = handler.handleGET(reqWithSession, route);
  assert(respWithSession.statusCode == 200);
  assert(respWithSession.headers.find("Set-Cookie") ==
         respWithSession.headers.end());
  std::cout << "    [OK] Existing session reused successfully\n";
